...
3. **`perRequestMemMb` provenance.** Per-node authored constant, or derived from request/workload size ([`request-type-model.md`](./request-type-model.md))? A workload-derived footprint is more honest but couples this spec to the request model.
