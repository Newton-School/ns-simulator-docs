...
the compiled row plan in the Question Studio walkthrough. For exhaustive field
syntax, component tokens, and examples, read `specs/test-case-catalog.md` from
the repository.
...
Runtime state scopes currently include request, delivery, broker, replication,
protocol, idempotency, commit-outcome, lock, and reservation. Verify exact states
in `questionAuthoringSemanticRules.ts` and `simulationSemantics.ts`.
...
The current visual Studio's advanced verdict selector is generated from
`RUBRIC_METRIC_CAPABILITIES` in `authoringCapabilities.ts`.
...
name. Use the registry or `specs/test-case-catalog.md`.
...
When available, use the repo's executable validators and tests rather than manual
JSON inspection alone. Useful paths include:

- `src/engine/analysis/questionAuthoringCompiler.ts`
- `src/engine/analysis/authoringValidator.ts`
- `scripts/validate-question-dir.ts`
- question-specific engine tests and fixtures.

Report schema/compile validation separately from behavioral good/bad-topology
validation.
