...
Use this reference to write the author-facing walkthrough. When repository source
is available, current renderer labels and compiler behavior override this summary.
...
passes `deserializeQuestionAuthoringArtifact` and the current project schema.
Legacy topology rubric metrics or an ID that does not match the title-derived slug
can make an import fail.
...
The codebase contains a Discrimination Lab (`prove`) stage, but it is currently
hidden from the rail. Do not instruct authors to click it unless current source
shows it has been re-enabled. Keep reference/gamed-design validation in the
...
## 11. Source map for current labels

- stages: `questionStudioStages.ts`
- shell/stage composition: `QuestionStudioShell.tsx`
- setup/metadata: `QuestionSetupEditor.tsx`, `QuestionMetadataEditor.tsx`
- prompt: `QuestionBriefEditor.tsx`, `PromptDetailsEditor.tsx`
- start/constraints: `LearnerStartEditor.tsx`, `QuestionConstraintsEditor.tsx`
- scenarios: `ScenarioSuiteEditor.tsx`
- grading: `StructuralGradingEditor.tsx`, `SemanticGradingEditor.tsx`,
  `MetricGradingEditor.tsx`, `RubricCheckEditor.tsx`
- preview/export: `LearnerExperiencePreview.tsx`, `GeneratedOutputPreview.tsx`
- compilation: `questionAuthoringCompiler.ts`
- project schema/import: `questionAuthoringProject.ts`,
  `questionAuthoringImport.ts`

If a control named in a generated walkthrough is absent from these sources, the
