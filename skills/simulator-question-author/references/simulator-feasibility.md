...
The authoritative registry is `src/engine/analysis/supportLedger.ts`.
...
The unified entrypoint is `src/engine/runSimulation.ts`.
...
The current analytic capacity order is defined in
...
Verify in the ledger, but these are generally reliable starting points:
