...
- **Traffic Source** (Templates) — this is *Users*.
...
|---|---|---|---|---|
| t3.medium *(burstable)* | 2 | 64 | ~50,000 | 40,000 |
...
- **Bigger boxes ⇒ fewer servers, ~the same cost.** You pay roughly per vCPU, so 25
  small / 13 medium / 7 large all land near ~$2.5/hr. Instance *size* is not where you
  save money here.
...
- **Stop when → "Request budget reached"**: a **Max requests** field appears. The run
  generates *exactly* that many requests and then drains — this is how you model
...
- **"Halt early if a node is saturated (≥100%)"**: an optional guard usable with
  ...
  report `stopReason: 'saturation'` and *when* it broke.
...
- **The 80% rule is a headroom budget, not a hard limit.** A server at 83% is still
  *stable* (it isn't dropping), but it has no burst headroom — which is why 12 servers
...
- **Smaller sale, real simulation:** drop Base RPS to e.g. 10,000 and set a
  ...
  *discrete* per-request engine (real retries, queue jitter) instead of the analytic
