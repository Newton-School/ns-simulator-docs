...
> Do not start by opening
> `src/engine/analysis/fixtures/quickcart-flash-sale.question.json`. That fixture
> predates the current Question Studio project contract: its learner-facing title
> does not derive its stored ID, and it uses legacy topology rubric metrics. The
> steps below express the same intent using supported Studio controls.
